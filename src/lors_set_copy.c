...
void _lorsRemoveDepots(JRB jrb_tree,IBP_depot *depots,int ndepots)
{
   JRB node;
   int i;
   LorsDepot *dp;
...
_lorsCreateMCopyNE( __LorsJobPool **jp,
    ...
    *jp = job_pool; 
    ...
    {
        fprintf(stderr, "off: %lld len: %lld\n", off, length);
...
next_mapping:
        ...
        if ( g_lors_demo )
        {
          ...
          fprintf(stderr,"DRAW MappingBegin %s:%4d %ld %lld %lld %s:%4d\n", 
             ...
             dst_lm->depot.host,dst_lm->depot.port);
#endif
          fprintf(stderr,"DRAW Arrow3 from %s:%4d %lu to %s:%4d 10\n",
               src_lm->depot.host,src_lm->depot.port, dst_lm->id,
               dst_lm->depot.host,dst_lm->depot.port);
            };
        }
        ...
        if ( nwrite <= 0){
            ...
            if ( jrb_node == jrb_nil(lm_tree)){
               ...
               dll_traverse(tmplist,dst_mappings){
                     dst_lm = (LorsMapping *)jval_v(tmplist->val);
                     dst_lm->id = _lorsIDFromCap(dst_lm->capset.readCap);
                     fprintf(stderr,"DELETE Arrow3 from %s:%4d %lu to %s:%4d \n",
                                    src_lm->depot.host,src_lm->depot.port, dst_lm->id,
                                    dst_lm->depot.host,dst_lm->depot.port);
                     fflush(stderr);
               };
...
next_mapping:
     ...
     dll_traverse(tmplist,dst_mappings){
          ...
          fflush(stderr);
#else
          fprintf(stderr,"DRAW MappingBegin %s:%4d %lu %lld %ld %s:%4d\n",
             src_lm->depot.host,src_lm->depot.port,dst_lm->id,
             src_lm->exnode_offset,src_lm->logical_length,
             dst_lm->depot.host,dst_lm->depot.port);
#endif
     ...
     if ( g_lors_demo )
     {
      dll_traverse(tmplist,dst_mappings){
             ...
             fprintf(stderr,"DRAW MappingEnd %d %I64d %ld %s:%4d\n",
                             ...
                             dst_lm->depot.host,dst_lm->depot.port);
#else
             fprintf(stderr,"DRAW MappingEnd %lu %lld %ld %s:%4d\n",
                             dst_lm->id,src_lm->exnode_offset,src_lm->logical_length,
                             dst_lm->depot.host,dst_lm->depot.port);
#endif
             fflush(stderr);
...
next_mapping:
        ...
        lorsDemoPrint(stderr,"DRAW Arrow3 from %s:%4d %lu to %s:%4d 10\n",
               src_lm->depot.host,src_lm->depot.port, dst_mapping->id,
               dst_mapping->depot.host,dst_mapping->depot.port);
#ifdef _MINGW
        lorsDemoPrint(stderr,"DRAW MappingFrom %s:%4d %I64d %I64d %lu %I64d %ld %s:%4d 10\n",
               ...
               dst_mapping->depot.host,dst_mapping->depot.port);
#else
        lorsDemoPrint(stderr,"DRAW MappingFrom %s:%4d %lld %lld %lu %lld %ld %s:%4d 10\n",
               src_lm->depot.host,src_lm->depot.port,
               src_lm->exnode_offset+p_off_in_mapping+n_done, p_len,
               dst_mapping->id, dst_mapping->exnode_offset, 
               dst_mapping->logical_length, 
        ...
        while ( n_done < p_len)
        {
            ...
            if ( nwrite == 0){
                ...
                if ( jrb_node == jrb_nil(lm_tree)){
                    ...
                    if ( g_lors_demo )
                    {
                    lorsDemoPrint(stderr,"DELETE Arrow3 from %s:%4d %lu to %s:%4d \n",
                        src_lm->depot.host,src_lm->depot.port, dst_mapping->id,
                        dst_mapping->depot.host,dst_mapping->depot.port);
                        fflush(stderr);
                    }
...
next_mapping:
     ...
     if ( g_lors_demo )
     {
       ...
       lorsDemoPrint(stderr,"DRAW MappingFrom %s:%4d %I64d %ld %d %I64d %ld %s:%4d 10\n",
               ...
               dst_lm->depot.host,dst_lm->depot.port);
#else
       lorsDemoPrint(stderr,"DRAW MappingFrom %s:%4d %lld %lu %lu %lld %ld %s:%4d 10\n",
               src_lm->depot.host,src_lm->depot.port,
               src_lm->exnode_offset+p_off_in_mapping+n_done, p_len,
               dst_lm->id, dst_lm->exnode_offset, 
               dst_lm->logical_length, 

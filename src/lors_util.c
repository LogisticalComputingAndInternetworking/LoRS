                         ...
                         ulong_t  e2e_bs)
{
    ...
    } else 
    {
    	...
    	int p_blocks;
        ulong_t l_blocks, p_last_length, l_last_length;
        block_length_count(alloc_length, blocksize, &p_blocks, &p_last_length, 16);
...
IBP_depot lors_strtodepot(char *hostname)
{
    char *s;
    IBP_depot ibpdepot;
    ibpdepot = (IBP_depot) calloc(1, sizeof(struct ibp_depot));

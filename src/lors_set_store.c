                     ...
                     int opts)
{
    ...
    dll_traverse(dl_node,jp_store->job_list)
    {
        ...
        {
            fprintf(stderr, "failed commit on mapping: 0x%p: %d\n", lm, job->status);
            /* TODO: try it again */
        }
    }
...
next_depot:
    ...
    do 
    {
        ...
        if ( (left_t = _lorsRemainTime(begin_time,job->timeout)) < 0 ) {
            fprintf(stderr, "begin: %ld timeout: %d left: %ld\n",
        ...
        //skip depot fetch if we are using proxy allocation, we don't need it
        ...
        lorsDebugPrint(1, "Capability  %24s:%4d %8lld %8ld\n", 
                            ...
                            lm->alloc_length);
#endif
        //LoDN PROXY ALLOCATION WILL USE NEW ALLOCATION FUNCTION
        if (lodn_allocation.type == LORS_PROXY_ALLOCATION) {
        	socaps = lodnProxyAllocate(ldepot->depot, &timer, lm->alloc_length, ibpattr);
        	lorsDebugPrint(D_LORS_VERBOSE, "lodnProxyAllocate RETURNED\n");
        } else {
...
/* STORE */
    ...
    if ( g_lors_demo )
    {
        fprintf(stderr,"MESSAGE 2 Storing...\n");
         ...
         fprintf(stderr,"DRAW Arrow1 fro %d %I64d %ld %s:%4d\n",
            ...
            ldepot->depot->host,ldepot->depot->port);
#else
        fprintf(stderr,"DRAW Arrow1 fro %lu %lld %ld %s:%4d\n",
            lm->id,lm->exnode_offset,lm->logical_length,
            ldepot->depot->host,ldepot->depot->port);
#endif
        fflush(stderr);
    ...
    while ( written != job->src_length)
    {
        if ( (left_t = _lorsRemainTime(begin_time,job->timeout)) < 0 ) 
        {
            fprintf(stderr, "2begin: %ld timeout: %d left: %ld\n",
                    begin_time, job->timeout, left_t);
            ret = LORS_TIMEOUT_EXPIRED;
            goto bail;
        }
            ...
            } else 
            {
                ...
                if ( g_lors_demo )
                {
                    ...
                    fprintf(stderr,"DELETE Arrow1 fro %d %I64d %ld %s:%4d\n",
                        ...
                        ldepot->depot->host,ldepot->depot->port);
#else
                    fprintf(stderr,"DELETE Arrow1 fro %lu %lld %ld %s:%4d\n",
                        lm->id,lm->exnode_offset,lm->logical_length,
                        ldepot->depot->host,ldepot->depot->port);
#endif
                    fflush(stderr);
...
bail:
	lorsDebugPrint(D_LORS_VERBOSE, "_lorsDoCreateStore Bailing...\n");
    if ( ret != LORS_SUCCESS ){
...
//POTENTIAL ERROR IN _lorsDoCondition:
//int ret is never initialized if lc_array is NULL, thus, the return 
//value of this function will be whatever data was at ret before it ran.
//This code behavior may produce incosistent results on different machines

                                  ...
                                  int       opts)
{
    ...
    for (i=0; src_list[i] != NULL; i++)
    {
        ...
        if ( node == NULL )
        {
            d = new_ibp_depot(src_list[i]->host, src_list[i]->port, 0);
	    d->rid = 0; /* Set the Resource ID to zero. */
    ...
    for (i=0; dst_list[i] != NULL; i++)
    {
        ...
        if ( node == NULL )
        {
            d = new_ibp_depot(dst_list[i]->host, dst_list[i]->port, 0);
            d->rid = 0; /* Set the Resource ID to zero. */
            if ( strcmp(d->host, "localhost") == 0 )
            {
    ...
    d = new_ibp_depot("localhost", 6714, 0);
                           ...
                           double *metric)
{
    ...
    JRB         node = NULL;
    LboneResolutionIndex    *li;
    struct ibp_depot         s_depot = {"localhost", 6714, 0};
                                      ...
                                      LboneResolution *lr, int opts)
{
    ...
    int             lret = -1;
    double          metric = 0.0;
    struct ibp_depot        localhost = {"localhost", 6714, 0};

    ...
    do {
        ...
        while ( job_node != NULL ) {
            ...
            if ( jp->buf != NULL ) { /* write into internal buffer */
                ...
                {
#ifdef _MINGW
                    fprintf(stderr,"DRAW Output %I64d %lld white\n",
...
void  *_lorsWorkThread (void *v)
{
     ...
     while ( 1 )
     {
        ...
        switch ( job->type ){
        //J_CREATE jobs created in: 
        //lors_set_alloc.c:lorsSetAllocate()
        //lors_set_copy.c:_lorsSetAllocateByBlockArray()
        //J_CREATE_STORE jobs created in:
        //lors_set_store.c:lorsSetStore()
        case J_CREATE:
        	printf("ALLOCATION: %d\n",lodn_allocation.type);
        	if (lodn_allocation.type==1)
        	{
        		printf("J_CREATE\n");
        		exit(0);
        	}

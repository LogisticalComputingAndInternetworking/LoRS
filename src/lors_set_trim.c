    ...
    */
    ...
    /*{*/
            ...
            continue;
        {
            ...
            node = node_tmp;
#endif
            fprintf(stderr, "Removing mapping: 0x%p at offset: %lld length: %lu\n",  lm, lm->exnode_offset, lm->logical_length);
    ...
    {
        lm = (LorsMapping *)jval_v(node->val);
        fprintf(stderr, "0x%p mapping still in mapping_map.\n", lm);
    }
#endif /* end DEBUG */
        ...
        if ( lma != NULL )
        {
            for (i=0; i < 2; i++ )
            {
                ...
                if (lm != NULL ) 
                {
                    ...
                    fprintf(stderr, "Returning mapping: 0x%p at offset: %lld length: %lu	\n",  lm, lm->exnode_offset, lm->logical_length);
...
int _lorsDoTrim( __LorsJob *job, __LorsThreadInfo *tinfo)
{
...
#else
                lorsDemoPrint(stderr, "DELETE Mapping %lu %lld %ld %s:%d\n",
                    lm->id, lm->exnode_offset, lm->logical_length, 
                    lm->depot.host, lm->depot.port);
#endif
                fflush(stderr);
                ...
                job->src_offset+job->src_length >= lm->exnode_offset +lm->logical_length)
    {
        ...
        }  else if  (opts & LORS_TRIM_ALL )
        {
            ...
            fprintf(stderr, "trim bottom\n");
            fprintf(stderr, "lm: 0x%p off: %lld len: %lu\n",
                     lm, lm->exnode_offset, lm->logical_length);
                ...
                lm->exnode_offset+lm->logical_length > job->src_offset+job->src_length)
    {
        ...
        }  else if  (opts & LORS_TRIM_ALL )
        {
            ...
            fprintf(stderr, "lm2: 0x%p off: %lld len: %lu\n",

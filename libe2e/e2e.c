...
char* CalculateChecksum(unsigned char *ablock, int size)
{
  int lenMD5;
  MD5_CTX context;
  unsigned char checksum[CH_SIZE];
  char *hexChecksum;
...
#if 1
{
  ...
  *output_buffer = (char *)malloc(sizeof(unsigned char)*(physical_size+1));
  ...
  while ( length > 0 )
  {
      ...
      checksum = gen_block_checksum((unsigned char *)(input_buffer+nread), blocksize);
		       ...
		       unsigned long int *output_length)
{
  ...
  *output_buffer = (char *)malloc(sizeof(unsigned char)*((tblocks+1)*(tmemory)+1));
  if((*output_buffer) == NULL){
    retval = -1;
  ...
  *output_length = tblocks*(tmemory)+(last_length-CHECKSUM_H);
  ...
  while(length > 0 )
  {
      ...
      r = VerifyBlockChecksum((unsigned char *)(input_buffer+nread+CHECKSUM_H), checksum, blocksize-CHECKSUM_H);
      if(r != OK){
          retval=-1;
          free(*output_buffer);
          *output_buffer = NULL;
			    ...
			    unsigned long int e2ebs)
{
  ...
  switch(encryptType){
	...
	fprintf(stderr,"DES: Error trying to encrypt a block\n");
      break;
    case ENC_XOR:
      cryptBlock =(unsigned char *)encrypt_XOR(theBlock, thekey, e2ebs);
      if(cryptBlock == NULL) 
	fprintf(stderr,"XOR: Error trying to encrypt a block\n");
      break;
    case ENC_NONE:
      cryptBlock = (unsigned char *)malloc(sizeof(char *)*(e2ebs+1));
		   ...
		   int e2ebs)
{
  ...
  switch(encryptType){
	...
	fprintf(stderr,"DES: Error trying to decrypt a block\n");
      break;
    case ENC_XOR:
      clearBlock = decrypt_XOR((char *)cryptBlock,thekey,e2ebs);
      if(clearBlock == NULL)
	fprintf(stderr,"XOR: Error trying to decrypt a block\n");
      break;
    case ENC_NONE:
      clearBlock = (char *)cryptBlock;
      break;
  }
		       ...
		       char *key)
{
  ...
  tmpBlock = (char *)malloc(sizeof(unsigned char)*(input_blocksize+pad1+1));
  *output_buffer = (char *)malloc(sizeof(unsigned char)*
  ...
  *output_blocksize = input_blocksize+ENCRYPT_H+pad1;
  ...
  while ( length > 0 )
  {
      ...
      encBlock = (char *)encrypt_DES(tmpBlock, key, blocksize+pad);
		       ...
		       char *key)
{
  ...
  tmpBlock = (char *)malloc(sizeof(unsigned char)*((tmemory)+1));
  *output_buffer = (char *)malloc(sizeof(unsigned char)*
                            (tblocks*tmemory)+
  ...
  while ( length > 0 )
  {
      ...
      encBlock = decrypt_DES((unsigned char *)tmpBlock, key, blocksize-ENCRYPT_H);
      /* paste decrypted block */
		       ...
		       char *key)
{
  ...
  tmpBlock = (char *)malloc(sizeof(unsigned char)*(input_blocksize+1));
  *output_length = (tblocks *input_blocksize + last_length);
  *output_buffer = (char *)malloc(sizeof(unsigned char)*((*output_length)+1));
		       ...
		       char *key)
{
    ...
    *output_buffer = (unsigned char *)input_buffer;
    *output_blocksize = input_blocksize;
		       ...
		       char *key)
{
    ...
    *output_buffer = (char *)input_buffer;
    ...
    if ( key_len == 7 ) 
    {
        ...
        {
            blocksize = (input_blocksize < length ? input_blocksize : length);
            encBlock = new_decrypt_XOR((char *)(input_buffer+nread), key, blocksize);
            nread += blocksize;
            length -= blocksize;
        }
    } else if ( key_len == 4 )
    {
        new_int_XOR((char *	)input_buffer, key, input_length);
    } else {
        fprintf(stderr, "unknown key length: %d\n", key_len);
        retval = -1;
    }
		       ...
		       unsigned long int *output_blocksize)
{
  ...
  char temp[REAL_SIZE+1], val[REAL_SIZE];
  ...
  header = (unsigned char *)create_header(temp,COMPR_H);
		      ...
		      unsigned long int *output_length)
{
  ...
  temp[REAL_SIZE]='\0';
  strncpy(temp,(char *)input_buffer,REAL_SIZE);
  org_len = atol(temp);

  /*fprintf(stderr, "original length: %d: %s\n", org_len, temp);*/

  tmpBlock = (char *)malloc(sizeof(unsigned char)*((input_length-COMPR_H)+1));
  clear_buff = (char *)malloc(sizeof(unsigned char)*((org_len)+1));
  *output_buffer = (char *)malloc(sizeof(unsigned char)*((org_len)+1));
		     ...
		     int *header_size)
{
  ...
  char *header,temp[REAL_SIZE+1],val[REAL_SIZE];
  int diff,i=0;
  ...
  *e2emapping = (char *)malloc(sizeof(unsigned char)*((tblocks*(REAL_SIZE_H+e2ebs))+1));
		       ...
		       unsigned long int e2ebs)
{
  ...
  tblocks = physical_size/(e2ebs);
  tmpBlock = (char *)malloc(sizeof(unsigned char)*(e2ebs-REAL_SIZE_H+1));
  *userMapping = (char *)malloc(sizeof(unsigned char)*(tblocks*(e2ebs-REAL_SIZE_H)+1));
		       ...
		       char *key)
{
    ...
    *output_blocksize = input_blocksize+BLOCK_SIZE;
    ...
    fprintf(stderr, "output_length: %lu, output_blocksize: %lu\n",
    ...
    *output_buffer = (char *)calloc(*output_length, 1);
    ...
    aes_enc_key((unsigned char *)c_key, BLOCK_SIZE, ctx);
                     ...
                     char *buf, char *dbuf, aes_ctx *ctx)
{
    ...
    *output_length = input_length + BLOCK_SIZE;
    ...
    if (input_length <= BLOCK_SIZE)
    {
        ...
        aes_enc_blk((unsigned char *)(dbuf + BLOCK_SIZE), (unsigned char *)(dbuf + len), ctx);
		       ...
		       char *key)
{
    ...
    *output_buffer = (char *)calloc(*output_length, sizeof(char));
    ...
    output = *output_buffer;
    input = (char *)input_buffer;

    memcpy(c_key, key, BLOCK_SIZE);
    aes_dec_key((unsigned char *)c_key, BLOCK_SIZE, ctx);

    for (i=0; i < tblocks; i++)
    {
        ret = aes_DecryptBlock((unsigned char *)input, input_blocksize, output, &blocksize,
                buf1, buf2, dbuf, ctx);
        if ( ret != 0 )
        {
    ...
    ret = aes_DecryptBlock((unsigned char *)input, last_length, output, &blocksize,
                buf1, buf2, dbuf, ctx);
    if ( ret != 0 )
    {
        fprintf(stderr, "aes_DecryptBlock error\n");
               ...
               char *dbuf, aes_ctx *ctx)
{
    ...
    *output_length = input_length - BLOCK_SIZE;
    ...
    { 
        ...
        aes_dec_blk((unsigned char *)(dbuf + len), (unsigned char *)(dbuf + BLOCK_SIZE), ctx);

                    ...
                    
{
    ...
    fprintf(stderr, "length: %lu blocksize: %lu\n", rp->length, rp->blocksize);
...
void *lorsDesEnc(void *v)
{
    ...
    } else if ( e2e_type == 1 )
    {
        lret = new_xor_EncryptMapping(lrp->dec, lrp->length, lrp->blocksize, 
                              (unsigned char **)&(lrp->enc),&(lrp->new_length), &(lrp->new_blocksize),
                              lrp->key);
    } else if ( e2e_type == 2 ) {
...
void *lorsDesDec(void *v)
{
    ...
    if ( e2e_type == 0 )
    {
        lret = xor_DecryptMapping((unsigned char *)lrp->enc,lrp->new_length, lrp->new_blocksize,
            &(lrp->dec),&(lrp->length),lrp->key);
    } else if ( e2e_type == 1 ) 
    {
        lret = new_xor_DecryptMapping((unsigned char *)lrp->enc,lrp->new_length, lrp->new_blocksize,
            &(lrp->dec),&(lrp->length),lrp->key);
    } else if ( e2e_type == 2 ) {
        ...
        /*pthread_mutex_lock(&lock);*/
        lret = aes_DecryptMapping((unsigned char *)lrp->enc,lrp->new_length, lrp->new_blocksize,

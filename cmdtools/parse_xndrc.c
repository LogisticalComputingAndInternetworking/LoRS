...
{
    { "pound.eecs.utk.edu", 6714, 0},
    { "acre.eecs.utk.edu",  6714, 0},
    { "ounce.eecs.utk.edu", 6714, 0},
    { "",                     0, 0}
    #define DEPOT_CNT 3
};
...
int set_xndrc_defaults(XndRc *xndrc)
{
   ...
   /******till here ****/
    ...
    xndrc->allocation_type = LORS_NORMAL_ALLOCATION;
...
int read_xndrc_file(XndRc *xndrc, char *filename)
{
    ...
    while (get_line(is) >= 0)
    {
        ...
        else if (strcmp(is->fields[0], "LBONE_SERVER") == 0) 
        {
            ...
            dp = new_ibp_depot(is->fields[1], port, 0);
            dp->rid = 0;   /* Set the resource ID field to zero. */
        ...
        else if (strcmp(is->fields[0], "COPIES") == 0) 
        {
            ...
            if (is->NF != 2)
                continue;
            if ( strtotimeout(is->fields[1], (time_t*)&temp_size) == -1 )
            {
        ...
        else if(strcmp(is->fields[0],"ALLOCATE")==0) {
        	int port;
        	if (is->NF != 2)
        		continue;
        	sscanf(is->fields[1],"%d",&port);
        	if (port!=0) xndrc->allocation_type = LORS_PROXY_ALLOCATION;
        	else xndrc->allocation_type = LORS_NORMAL_ALLOCATION;
        }
        else if(strcmp(is->fields[0],"USERNAME")==0) {
        	if (is->NF != 2)
        		continue;
        	xndrc->username = strdup(is->fields[1]);
        }
        else if(strcmp(is->fields[0],"PASSWORD")==0) {
        	if (is->NF != 2)
        		continue;
        	xndrc->password = strdup(is->fields[1]);
        }
        else if(strcmp(is->fields[0],"LODN_URL")==0) {
        	if (is->NF != 2)
        		continue;
        	xndrc->lodn_url = strdup(is->fields[1]);
        }
    }
   ...
   /********temp***********/ 
    if( depot_list1 != NULL )
    {
        ...
        xndrc->depotList1 = (IBP_depot *)calloc((depot1_cnt+1),sizeof(IBP_depot));
    ...
    {
        if ( xndrc->depotList2 != NULL ) free(xndrc->depotList2);
        xndrc->depotList2 = (IBP_depot *)calloc((depot2_cnt+1),sizeof(IBP_depot));
#if 0
        if(depot_list1 == NULL)
        {

...
int main (int argc, char **argv)
{
    int i, ret;
    LorsExnode *exnode;
    struct ibp_depot   lbone_server = {"dsj.sinrg.cs.utk.edu", 6767, 0};
    ...
    int         fragments=0;
    char        *rcfile=NULL;
    char		*proxy_options = NULL;
    int			proxy = 0;
    int	 		proxy_override = 0;
  ...
  { "lodn-allocate", 0, POPT_ARG_STRING|POPT_ARGFLAG_OPTIONAL, &proxy_options, LODN_ALLOCATE,
	    "Turn on LoDN proxy allocation. The STRING value contains authentication credentials separated by an &.", NULL},
  { "normal-allocate",'n', POPT_ARG_NONE, &proxy_override, NORMAL_ALLOCATE,
		"Override LoDN proxy allocation if specified in the xndrc file.", NULL},
        POPT_AUTOHELP
        { NULL, 0, 0, NULL, 0 }
    };
    ...
    proxyAllocateInit(&lodn_allocation);
    setProxyAllocateXndrcOptions(xndrc, &proxy, &lodn_allocation);
    ...
    //set proxy allocation options from commandline
    if (setProxyAllocateOptions(proxy,proxy_override,proxy_options,&lodn_allocation)) {
    	fprintf(stderr,"Invalid option: --lodn-allocate=\"%s\"\n",proxy_options);
    }
    
    //TEMPORARY: take out when proxy allocation is fixed
        if (proxy && !proxy_override) {
        	puts("Proxy allocation is not supported yet. Please use standard allocation method.");
        	exit(EXIT_FAILURE);
    }
    
    //checks proxy allocation options
    if (proxy && !proxy_override && checkProxyAllocateOptions(lodn_allocation)) {
    	puts("Proxy allocation options error. Check xndrc file and/or commandline options.");
    	exit(EXIT_FAILURE);
    }

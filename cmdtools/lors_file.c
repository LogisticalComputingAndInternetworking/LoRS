...
void *lorsWriteFile ( void *para )
{
     ...
     while ( 1 ){
        ...
        fflush(stderr);
#else
        lorsDemoPrint(stderr, "DRAW Output %lld %d white\n",
                fjq->queue[fjq->tail].offset, fjq->queue[fjq->tail].size);
#endif
        }
                     ...
                     int        opts)
{
    ...
    if ( filename == NULL )
    {
	    ...
	    filename = val.s;   /* Assign the fileName. */
    }
...
failed_open:
    ...
    */
    ...
    while ( written < length )
    {
        ...
        if ( dret <= 0 )
        {
            fprintf(stderr, "lorsSetLoad failure. %lld\n", dret);
    ...
    do 
    {
        ...
        if ( g_lors_demo )
        {
            ...
            lorsDemoPrint(stderr,"DRAW MappingEnd %d %I64d %ld %s:%d\n",
                      ...
                      lm->depot.host,lm->depot.port);
#else
            lorsDemoPrint(stderr,"DRAW MappingBegin %s:%4d %lu %lld %ld %s:%4d\n",
                      lm->depot.host,lm->depot.port,
                      lm->id,lm->exnode_offset,lm->logical_length,
                      lm->depot.host,lm->depot.port);
            lorsDemoPrint(stderr,"DRAW MappingEnd %lu %lld %ld %s:%d\n",
                      lm->id,lm->exnode_offset,lm->logical_length,
                      lm->depot.host,lm->depot.port);
#endif
            fflush(stderr);

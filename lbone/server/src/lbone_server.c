...
/* ADDED: 05/27/2010 */
static char *get_lbone_pass() {
	char *lbone_pass = getenv("LDAP_PASS");
	if (lbone_pass == NULL) lbone_pass = LBONE_PASS;
	return lbone_pass;
}
    ...
    */
    ...
    while(1)
    {
        ...
        if (server->poll == 1)
        {
            ...
            retval = ldap_simple_bind_s (ldap, LBONE_LDAP, get_lbone_pass());
        ...
        sleep(5*60); /* poll every 45 minutes */
    }
}
...
DepotInfo *recv_depot_list(ClientInfo info, pthread_t tid, int src_cnt)
{
    ...
    ld_ret = ldap_simple_bind_s (ldap, LBONE_LDAP, get_lbone_pass());

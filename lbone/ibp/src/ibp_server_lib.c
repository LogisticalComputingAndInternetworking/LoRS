...
int boot(int argc, char **argv)
{
  ...
  /* Fixes a bug on Leopard where the rlim_cur must be the the min of these two*/ 
  #ifdef OPEN_MAX 
     ls_limit.rlim_cur = (OPEN_MAX < ls_limit.rlim_max) ? OPEN_MAX : ls_limit.rlim_max;
  #else
     ls_limit.rlim_cur = (sysconf(_SC_OPEN_MAX)<ls_limit.rlim_max) ? sysconf(_SC_OPEN_MAX) : ls_limit.rlim_max;
  #endif 
